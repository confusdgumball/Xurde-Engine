...
class PlayState extends MusicBeatState
{
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				var swagNote:Note;
				if (gottaHitNote){
					swagNote = new Note(daStrumTime, daNoteData, oldNote,false, boyfriend.style);
				}else {
					swagNote = new Note(daStrumTime, daNoteData, oldNote,false, dad.style);
				}
				...
				for (susNote in 0...Math.floor(susLength))
				{
					...
					if (gottaHitNote){
						sustainNote	= new Note(daStrumTime + (Conductor.stepCrochet * susNote) + (Conductor.stepCrochet / FlxMath.roundDecimal(SONG.speed, 2)), daNoteData, oldNote, true,boyfriend.style);
					}else{
						sustainNote	= new Note(daStrumTime + (Conductor.stepCrochet * susNote) + (Conductor.stepCrochet / FlxMath.roundDecimal(SONG.speed, 2)), daNoteData, oldNote, true,dad.style);
					}
	...
	private function generateStaticArrows(player:Int,style:String = "normal"):Void
	{
		for (i in 0...4)
		{
			...
			switch(style){
				case 'kapi':
					babyArrow.frames = Paths.getSparrowAtlas('noteStyles/kapi','shared');
					...
					switch (Math.abs(i))
					{
						...
						case 'normal' | 'nyan':
							babyArrow.frames = Paths.getSparrowAtlas('noteStyles/'+style,'shared');
							babyArrow.animation.addByPrefix('green', 'arrowUP');
							babyArrow.animation.addByPrefix('blue', 'arrowDOWN');
							babyArrow.animation.addByPrefix('purple', 'arrowLEFT');
							babyArrow.animation.addByPrefix('red', 'arrowRIGHT');
